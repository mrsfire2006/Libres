...
export function BookHero({ book }: { book: BookDto }) {
        ...
        <section className="container mx-auto px-4 py-8 max-w-4xl bg-background text-foreground">
            ...
            <div className="flex flex-col md:flex-row gap-8 mb-12">
                ...
                <div className="shrink-0 mx-auto md:mx-0">
                    <div className={`w-60 md:w-75 aspect-2/3 rounded-xl shadow-xl relative overflow-hidden bg-zinc-900`}>
                        ...
                        <div className="absolute inset-x-0 bottom-0 h-1/2 bg-linear-to-t from-black/80 to-transparent flex items-end p-6">
                            <h2 className="text-white font-serif font-bold text-2xl leading-tight">{book.name}</h2>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col flex-1">
                    <h1 className="text-3xl md:text-4xl font-serif font-bold mb-2 text-foreground">{book.name}</h1>
                    <p className="text-lg text-muted-foreground mb-4">
                        by <span className="text-primary hover:underline cursor-pointer">{book.authorName}</span>
                    </p>
